...
d3.csv("data/infogram_viz3_lang_rank.csv").then(function (data) {
  data.sort(function (a, b) {
    return +a.median_watch - +b.median_watch;
  });
  ...
  const y = d3
    ...
    .range([heightLoll, 0])
    .padding(1);
  svgLoll.append("g").call(d3.axisLeft(y).tickSize(0));
  ...
  const applySelection = function () {
    svgLoll.selectAll(".lollipop-mark").style("opacity", function (d) {
      return !selectedLanguage || d.primary_language === selectedLanguage
        ? 1
        : 0.2;
    });
    svgLoll.selectAll(".lollipop-dot").attr("r", function (d) {
  ...
  svgLoll
    ...
    .attr(
      "class",
      (d) => "lollipop-mark lollipop-line " + langClass(d.primary_language),
    )
  ...
  svgLoll
    ...
    .attr(
      "class",
      (d) =>
        "lollipop-mark lollipop-dot dot_" +
        langClass(d.primary_language) +
        " " +
        langClass(d.primary_language),
    )
